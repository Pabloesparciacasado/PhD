...
# ─── CLASE: Security Price ──────────────────────────────────────────────────────

class SecurityPrice:
    """
    Carga y consulta de Security Price (IvyDB GI.ALL.IVYSECPRD).

    Estructura del dato:
        SecurityID | Date | Exchange | Currency | Bid | Ask | OpenPrice |
        ClosePrice | Volume | TotalReturn | AdjustmentFactor | AdjustmentFactor2
    ...
    _NAMES_IVYSECPRD: list[str] = [
        'SecurityID', 'Date', 'Exchange', 'Currency', 'Bid', 'Ask',
        'OpenPrice', 'ClosePrice', 'Volume', 'TotalReturn',
        'AdjustmentFactor', 'AdjustmentFactor2',
    ]

    # Dtypes compactos usados durante la compilación TXT → CSV.
    _DTYPE_IVYSECPRD: dict[str, str] = {
        'SecurityID':       'int32',
        'Exchange':         'int16',
        'Currency':         'int16',
        'Bid':              'float32',
    ...
    def cargar_datos(
        ...
        """
        Compila todos los archivos .txt de la carpeta IVYSECPRD y guarda el
        resultado como CSV. Las consultas se ejecutan directamente sobre el CSV
        con DuckDB (sin cargarlo en RAM).

        :param carpeta:    Ruta raíz (contiene la subcarpeta GI.ALL.IVYSECPRD).
        ...
        self._loader.compilar_carpeta(
            os.path.join(carpeta, "GI.ALL.IVYSECPRD"),
            guardar_en = guardar_en,
            dtype      = self._DTYPE_IVYSECPRD,
            names      = self._NAMES_IVYSECPRD,
            limite     = limite,
        )
    ...
    def cargar_csv(self, ruta: str) -> None:
        ...
        :param ruta: Ruta al CSV compilado (p.ej. security_price.csv).
        """
    ...
    def exportar_parquet(self, ruta: str) -> None:
        ...
        :param ruta: Ruta de destino del Parquet (p.ej. security_price.parquet).
        """
    ...
    def cargar_parquet(
        ...
        Ejemplo:
            sp.cargar_parquet(
                ruta     = "security_price.parquet",
                desde    = "2010-01-01",
                hasta    = "2023-12-31",
                columnas = ['SecurityID', 'Date', 'ClosePrice', 'Volume'],
            )
            sp.df.groupby('SecurityID')['ClosePrice'].last()   # pandas puro
        """
        if not os.path.exists(ruta):
            raise FileNotFoundError(f"No se encontró: {ruta}")
    ...
    def query(self, sql: str) -> pd.DataFrame:
        ...
        Ejemplo:
            sp.query("SELECT SecurityID, AVG(ClosePrice) FROM {csv} GROUP BY SecurityID")
        """
        self._verificar_datos()
...
if __name__ == "__main__":
    ...
    CSV     = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\security_price.csv"
    PARQUET = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\security_price.parquet"
    CARPETA = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado"

    sp = SecurityPrice(sep='\t')

    # ── Paso 1 (una sola vez): compilar TXT → CSV ─────────────────────────────
    # sp.cargar_datos(carpeta=CARPETA, guardar_en=CSV)

    # ── Paso 2 (una sola vez): CSV → Parquet  (más rápido y pequeño) ──────────
    # sp.cargar_csv(CSV)
    # sp.exportar_parquet(PARQUET)
    ...
    sp.cargar_parquet(
        ...
        columnas = [                 # elige solo las columnas que vayas a usar
            'SecurityID', 'Date', 'ClosePrice', 'Volume',
            'TotalReturn', 'AdjustmentFactor',
        ],
    )
