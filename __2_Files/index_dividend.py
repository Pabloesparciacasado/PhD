...
# ─── CLASE: Index Dividend ──────────────────────────────────────────────────────

class IndexDividend:
    """
    Carga y consulta de Index Dividend (IvyDB GI.ALL.IVYIDXDVD).

    Estructura del dato:
        SecurityID | Date | Expiration | Rate

    Contiene las tasas de dividendo implícitas para índices, por fecha de
    observación y fecha de vencimiento.
    ...
    _NAMES_IVYIDXDVD: list[str] = [
        'SecurityID', 'Date', 'Expiration', 'Rate',
    ]

    # Dtypes compactos usados durante la compilación TXT → CSV.
    _DTYPE_IVYIDXDVD: dict[str, str] = {
        'SecurityID': 'int32',
    ...
    def cargar_datos(
        ...
        """
        Compila todos los archivos .txt de la carpeta IVYIDXDVD y guarda el
        resultado como CSV. Las consultas se ejecutan directamente sobre el CSV
        con DuckDB (sin cargarlo en RAM).

        :param carpeta:    Ruta raíz (contiene la subcarpeta GI.ALL.IVYIDXDVD).
        ...
        self._loader.compilar_carpeta(
            os.path.join(carpeta, "GI.ALL.IVYIDXDVD"),
            guardar_en = guardar_en,
            dtype      = self._DTYPE_IVYIDXDVD,
            names      = self._NAMES_IVYIDXDVD,
            limite     = limite,
        )
    ...
    def cargar_csv(self, ruta: str) -> None:
        ...
        :param ruta: Ruta al CSV compilado (p.ej. index_dividend.csv).
        """
    ...
    def exportar_parquet(self, ruta: str) -> None:
        ...
        :param ruta: Ruta de destino del Parquet (p.ej. index_dividend.parquet).
        """
    ...
    def cargar_parquet(
        ...
        Ejemplo:
            idx.cargar_parquet(
                ruta     = "index_dividend.parquet",
                desde    = "2010-01-01",
                hasta    = "2023-12-31",
            )
            idx.df.groupby('SecurityID')['Rate'].mean()   # pandas puro
        """
        if not os.path.exists(ruta):
            raise FileNotFoundError(f"No se encontró: {ruta}")
    ...
    def query(self, sql: str) -> pd.DataFrame:
        ...
        Ejemplo:
            idx.query("SELECT SecurityID, AVG(Rate) FROM {csv} GROUP BY SecurityID")
        """
        self._verificar_datos()
...
if __name__ == "__main__":
    ...
    CSV     = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\index_dividend.csv"
    PARQUET = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\index_dividend.parquet"
    CARPETA = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado"

    idx = IndexDividend(sep='\t')

    # ── Paso 1 (una sola vez): compilar TXT → CSV ─────────────────────────────
    # idx.cargar_datos(carpeta=CARPETA, guardar_en=CSV)

    # ── Paso 2 (una sola vez): CSV → Parquet  (más rápido y pequeño) ──────────
    # idx.cargar_csv(CSV)
    # idx.exportar_parquet(PARQUET)
