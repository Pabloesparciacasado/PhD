...
# ─── CLASE: Std Option Price ────────────────────────────────────────────────────

class StdOptionPrice:
    """
    Carga y consulta de Standardized Option Price (IvyDB GI.ALL.IVYSTDOPD).

    Estructura del dato:
        SecurityID | Date | Days | ForwardPrice | Strike | CallPut | Premium |
        ImpliedVol | Delta | Gamma | Theta | Vega | Currency
    ...
    A diferencia de IVYOPPRCD, las opciones están estandarizadas por Days y
    Strike relativo (no son contratos individuales), lo que resulta en un
    dataset más compacto.

    Requiere: pip install duckdb
    """

    # Nombres de columna en el orden que tienen los TXT (archivos sin cabecera).
    _NAMES_IVYSTDOPD: list[str] = [
        'SecurityID', 'Date', 'Days', 'ForwardPrice', 'Strike', 'CallPut',
        'Premium', 'ImpliedVol', 'Delta', 'Gamma', 'Theta', 'Vega', 'Currency',
    ]

    # Dtypes compactos usados durante la compilación TXT → CSV.
    _DTYPE_IVYSTDOPD: dict[str, str] = {
        'SecurityID':  'int32',
        'Days':        'int16',
    ...
    def cargar_datos(
        ...
        """
        Compila todos los archivos .txt de la carpeta IVYSTDOPD y guarda el
        resultado como CSV. Las consultas se ejecutan directamente sobre el CSV
        con DuckDB (sin cargarlo en RAM).

        :param carpeta:    Ruta raíz (contiene la subcarpeta GI.ALL.IVYSTDOPD).
        ...
        self._loader.compilar_carpeta(
            os.path.join(carpeta, "GI.ALL.IVYSTDOPD"),
            guardar_en = guardar_en,
            dtype      = self._DTYPE_IVYSTDOPD,
            names      = self._NAMES_IVYSTDOPD,
            limite     = limite,
        )
    ...
    def cargar_csv(self, ruta: str) -> None:
        ...
        :param ruta: Ruta al CSV compilado (p.ej. std_option_price.csv).
        """
    ...
    def exportar_parquet(self, ruta: str) -> None:
        ...
        :param ruta: Ruta de destino del Parquet (p.ej. std_option_price.parquet).
        """
    ...
    def cargar_parquet(
        ...
        Ejemplo:
            sop.cargar_parquet(
                ruta     = "std_option_price.parquet",
                desde    = "2015-01-01",
                hasta    = "2023-12-31",
                columnas = ['SecurityID', 'Date', 'Days', 'Strike',
                            'CallPut', 'ImpliedVol', 'Delta'],
            )
            sop.df.groupby('Days')['ImpliedVol'].mean()   # pandas puro
        """
        if not os.path.exists(ruta):
            raise FileNotFoundError(f"No se encontró: {ruta}")
    ...
    def resumen(self) -> None:
        ...
        sql = f"""
            SELECT
                ...
                COUNT(DISTINCT Days)       AS tenores
            FROM {self._fuente()}
        """
        ...
        print(f"Tenores únicos : {int(r['tenores'])}")
    ...
    def query(self, sql: str) -> pd.DataFrame:
        ...
        Ejemplo:
            sop.query("SELECT Days, AVG(ImpliedVol) AS iv FROM {csv} GROUP BY Days ORDER BY Days")
        """
        self._verificar_datos()
...
if __name__ == "__main__":
    ...
    CSV     = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\std_option_price.csv"
    PARQUET = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\std_option_price.parquet"
    CARPETA = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado"

    sop = StdOptionPrice(sep='\t')

    # ── Paso 1 (una sola vez): compilar TXT → CSV ─────────────────────────────
    # sop.cargar_datos(carpeta=CARPETA, guardar_en=CSV)

    # ── Paso 2 (una sola vez): CSV → Parquet  (más rápido y pequeño) ──────────
    # sop.cargar_csv(CSV)
    # sop.exportar_parquet(PARQUET)
