...
# ─── CLASE: Historical Volatility ──────────────────────────────────────────────

class HistoricalVolatility:
    """
    Carga y consulta de Historical Volatility (IvyDB GI.ALL.IVYHISTVOLD).

    Estructura del dato:
        SecurityID | Date | Days | Currency | Volatility

    Contiene la volatilidad histórica realizada de cada subyacente, calculada
    sobre ventanas de Days días de retornos pasados.
    ...
    _NAMES_IVYHISTVOLD: list[str] = [
        'SecurityID', 'Date', 'Days', 'Currency', 'Volatility',
    ]

    # Dtypes compactos usados durante la compilación TXT → CSV.
    _DTYPE_IVYHISTVOLD: dict[str, str] = {
        'SecurityID': 'int32',
        'Days':       'int16',
        'Currency':   'int16',
        'Volatility': 'float32',
    ...
    def cargar_datos(
        ...
        """
        Compila todos los archivos .txt de la carpeta IVYHISTVOLD y guarda el
        resultado como CSV. Las consultas se ejecutan directamente sobre el CSV
        con DuckDB (sin cargarlo en RAM).

        :param carpeta:    Ruta raíz (contiene la subcarpeta GI.ALL.IVYHISTVOLD).
        ...
        self._loader.compilar_carpeta(
            os.path.join(carpeta, "GI.ALL.IVYHISTVOLD"),
            guardar_en = guardar_en,
            dtype      = self._DTYPE_IVYHISTVOLD,
            names      = self._NAMES_IVYHISTVOLD,
            limite     = limite,
        )
    ...
    def cargar_csv(self, ruta: str) -> None:
        ...
        :param ruta: Ruta al CSV compilado (p.ej. historical_volatility.csv).
        """
    ...
    def exportar_parquet(self, ruta: str) -> None:
        ...
        :param ruta: Ruta de destino del Parquet (p.ej. historical_volatility.parquet).
        """
    ...
    def cargar_parquet(
        ...
        Ejemplo:
            hv.cargar_parquet(
                ruta     = "historical_volatility.parquet",
                desde    = "2010-01-01",
                hasta    = "2023-12-31",
                columnas = ['SecurityID', 'Date', 'Days', 'Volatility'],
            )
            hv.df.groupby('Days')['Volatility'].mean()   # pandas puro
        """
        if not os.path.exists(ruta):
            raise FileNotFoundError(f"No se encontró: {ruta}")
    ...
    def resumen(self) -> None:
        ...
        sql = f"""
            SELECT
                ...
                COUNT(DISTINCT Days)       AS ventanas
            FROM {self._fuente()}
        """
        ...
        print(f"Ventanas únicas : {int(r['ventanas'])}")
    ...
    def query(self, sql: str) -> pd.DataFrame:
        ...
        Ejemplo:
            hv.query("SELECT SecurityID, Days, AVG(Volatility) FROM {csv} GROUP BY SecurityID, Days")
        """
        self._verificar_datos()
...
if __name__ == "__main__":
    ...
    CSV     = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\historical_volatility.csv"
    PARQUET = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado\historical_volatility.parquet"
    CARPETA = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\Acumulado"

    hv = HistoricalVolatility(sep='\t')

    # ── Paso 1 (una sola vez): compilar TXT → CSV ─────────────────────────────
    hv.cargar_datos(carpeta=CARPETA, guardar_en=CSV)

    # ── Paso 2 (una sola vez): CSV → Parquet  (más rápido y pequeño) ──────────
    hv.cargar_csv(CSV)
    hv.exportar_parquet(PARQUET)
    ...
    hv.cargar_parquet(
        ...
        columnas = [                 # elige solo las columnas que vayas a usar
            'SecurityID', 'Date', 'Days', 'Volatility',
        ],
    )
