...
COLUMNAS = [
    'OptionID','SecurityID', 'Date', 'Expiration','Last', 'Strike', 'CallPut', 'Bid', 'Ask', 'Volume', 'OpenInterest','AMSettlement','ExpiryIndicator',
...
PARQUET_FP = os.path.join(PARQUET_RUTA, "forward_price_filtered.parquet")
fp = ForwardPrice()
fp.cargar_parquet(PARQUET_FP,desde, hasta)
fp_df = fp.df

PARQUET_ZC = os.path.join(PARQUET_RUTA, "zero_curve.parquet")
zc  = ZeroCurve(sep='\t')
