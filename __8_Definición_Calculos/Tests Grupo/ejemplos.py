...
def attach_oracle(gg, p):
    ...
    out["abs_r01"] = np.abs((out["S_l"] - out["S_l2"]) )
    out["abs_r02"] = np.abs((out["S"] - out["S_l2"]) )
...
def resumen(out, umbral_gamma=1e-10, umbral_spot = 1e-2):
    rows = []
...
panel2 = build_config_panel(base,"A",p)
d2, gg2 = estimador_op1(panel2)
tablas.append(resumen(attach_oracle(gg2, p)) )
