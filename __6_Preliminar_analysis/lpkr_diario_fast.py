# In[]:
"""
LPR Bivariado — Modo B Diario (Fast)
======================================
Estima LPR para cada día usando solo las observaciones de ese día como pool.
Pre-filtrado por soporte del kernel Epanechnikov para eficiencia.
...
MIN_OI_CELL = 1      # OI mínimo en celda para estimar delta empírica
...
#kernel separable o indpendiente
def kernel_2d(u_m, u_tau, kernel="epanechnikov"):
    return kernel_1d(u_m, kernel) * kernel_1d(u_tau, kernel)


def lpr_bivariate_fast(m0, tau0,
                        m_sub, tau_sub, Y_sub, w_oi_sub,
                        h_m=H_M, h_tau=H_TAU,
                        kernel=KERNEL,
                        min_obs=MIN_OBS,
                        w_threshold=W_THRESHOLD):
    """
    LPR bivariado grado 2 sobre subconjunto pre-filtrado.
    ...
    # Kernel sobre subconjunto pre-filtrado
    u_m   = (m_sub   - m0) / h_m
    ...
    # Variables estandarizadas — evita mal condicionamiento (multicolinealidad)
    dm_std   = (m_sub   - m0) / h_m
    dtau_std = (tau_sub - tau0) / h_tau
...
# In[]:
# =============================================================================
# 4. MODO B — LPR DIARIO FAST
# =============================================================================

print("\nEstimando LPR diario (modo B fast)...")
...
for idx_d, date in enumerate(dates):
    ...
    df_day["w_oi_vol"] = (df_day["w_oi_vol"] / df_day["w_oi_vol"].max()).clip(lower=1e-6)

    # Arrays del pool del día
    ...
    for i in range(len(eval_day)):
        ...
        # PRE-FILTRO por soporte Epanechnikov — clave para velocidad
        mask = (
            (m_day   >= m0   - H_M)   & (m_day   <= m0   + H_M) &
            (tau_day >= tau0 - H_TAU) & (tau_day <= tau0 + H_TAU)
        )
        ...
        if mask.sum() < MIN_OBS:
            ...
            res = lpr_bivariate_fast(
                m0, tau0,
                m_day[mask], tau_day[mask],
                Y_day[mask], w_day[mask]
            )
...
# Cambio diario del precio de la opción por contrato
...
n_days_valid = df_emp["Date"].nunique()
print(f"Días con |ret_S| >= {MIN_RET_S*100:.1f}%: {n_days_valid:,}")
...
# Función de estimación por celda
...
# Agregar delta LPR por bucket para comparar
...
fig.suptitle(f"LPR Diario Modo B Fast — {OPTION_TYPE} — 2023\n"
...
# Delta empírica vs LPR — scatter por bucket ATM
ax = axes[1]
...
plt.tight_layout()
fname_plot = f"{PATH_OUTPUT}\\lpr_vs_emp_{OPTION_TYPE}_2023.png"
...
fname_real = f"{PATH_OUTPUT}\\lpr_real_2023_{OPTION_TYPE}.parquet"
df.to_parquet(fname_real, index=False)

fname_emp = f"{PATH_OUTPUT}\\delta_emp_2023_{OPTION_TYPE}.csv"
delta_by_cell.to_csv(fname_emp, index=False)

fname_gex = f"{PATH_OUTPUT}\\gex_daily_2023_{OPTION_TYPE}.csv"
