...
def build_pivot(cp_flag, value_col="gamma"):
...
pivot_c,    cutoff_C    = build_pivot("C", "gamma")
pivot_p,    cutoff_P    = build_pivot("P", "gamma")
pivot_c_bs, _           = build_pivot("C", "gamma_bs")
...
# In[]
# ============================================================
# 2. GRÁFICO 3D: Gamma BS de Calls por Fecha y Moneyness
# ============================================================
...
plot_3d(pivot_c_bs, "Gamma BS de Calls por Fecha y Moneyness", "gamma_bs")


# ============================================================
# 3. GRÁFICO 3D: Calls vs Puts — Gamma por Fecha y Moneyness
# ============================================================
...
for i, (piv, label, cmap) in enumerate(
    ...
    ax.set_xlabel("Fecha", labelpad=10)
    ax.set_ylabel("Moneyness (S/K)", labelpad=10)
    ax.set_zlabel("gamma", labelpad=10)
    ax.set_title(f"gamma — {label}", pad=12)
    fig.colorbar(surf, ax=ax, shrink=0.4, aspect=10, label="gamma")

plt.suptitle("Comparación gamma: Calls vs Puts por Fecha y Moneyness", fontsize=13, y=1.01)
plt.tight_layout()
plt.show()
...
# In[]
# ============================================================
# 4. Series temporales: gamma interpolado en cuantiles de moneyness
# ============================================================
...
def interp_group(group, cp):
    ...
    return pd.Series({
        row.label: np.interp(row.log_moneyness, group["log_moneyness"], group["gamma"])
...
PARQUET_OUTPUT = r"C:\Users\pablo.esparcia\Documents\OptionMetrics\output\time_series\quantile_gamma.parquet"
